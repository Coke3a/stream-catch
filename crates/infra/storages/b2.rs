...
impl StorageClient for B2StorageClient {
    ...
    async fn delete_object(&self, object_key: &str) -> Result<()> {
        B2StorageClient::delete_object(self, object_key).await
    }
}
...
fn map_delete_object_error(
    ...
    if let SdkError::ServiceError(service_err) = &err {
        ...
        let mut detail = format!(
            "failed to delete Backblaze B2 object (status {}, code {})",
            status, code
        );
    ...
    anyhow::Error::new(err).context("failed to delete Backblaze B2 object")
}
